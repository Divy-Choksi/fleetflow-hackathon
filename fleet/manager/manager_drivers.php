...
if (isset($_GET['update_status']) && isset($_GET['id'])) {
    ...
    $allowed_status = ['on_duty','off_duty','suspended','on_trip'];

    if (in_array($_GET['update_status'], $allowed_status)) {
        $stmt = $conn->prepare("UPDATE drivers SET status = ? WHERE driver_id = ?");
        $stmt->execute([$_GET['update_status'], $_GET['id']]);
    }

    header("Location: manager_driver_management.php");
    exit();
}

// 📥 Fetch all drivers
$stmt = $conn->query("SELECT * FROM drivers ORDER BY driver_id DESC");
        ...
        <main class="flex-1 overflow-y-auto p-8 flex flex-col">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <a href="add_driver.php" class="btn-premium px-5 py-2.5 rounded-xl text-sm font-bold">
    Add New Driver
</a>
                </div>
            </div>
...
<td class="py-4 pl-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <p class="font-medium text-white"><?= htmlspecialchars($d['full_name']); ?></p>
            <p class="text-[11px] text-gray-500">ID: #DRV-<?= $d['driver_id']; ?></p>
        </div>
    </div>
</td>

<td class="py-4 px-4">
    <p class="font-mono text-gray-200"><?= htmlspecialchars($d['license_number']); ?></p>
    <p class="text-xs text-gray-400"><?= htmlspecialchars($d['phone']); ?></p>
</td>

<td class="py-4 px-4 text-center">
<?php if ($days > 30): ?>
    <span class="text-status-success text-xs font-bold">Valid (<?= $days ?> days)</span>
<?php elseif ($days > 0): ?>
    <span class="text-status-warning text-xs font-bold">Expiring Soon (<?= $days ?> days)</span>
<?php else: ?>
    <span class="text-status-danger text-xs font-bold">Expired</span>
<?php endif; ?>
</td>

<td class="py-4 px-4 text-gray-300">
    <?= htmlspecialchars($d['license_category']); ?>
</td>
...
<td class="py-4 pr-6 text-right">
    <a href="?update_status=on_duty&id=<?= $d['driver_id']; ?>" class="text-green-400">✔</a>
    <a href="?update_status=off_duty&id=<?= $d['driver_id']; ?>" class="text-yellow-400 ml-2">☕</a>
    <a href="?update_status=suspended&id=<?= $d['driver_id']; ?>" class="text-red-400 ml-2">🚫</a>
</td>
